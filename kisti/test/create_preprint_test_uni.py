...
from osf import create_file_wb as cf
...
author = bulk_create_user.create_user_email_dict()
...
for data in datas:
    ...
    user_id = create_preprints.get_json_id(user_data.text)
    if user_id is None:
    ...
    if nodes_data.status_code > 300:
        ...
        node_id = create_preprints.get_json_id(nodes_data.text)


    #create file
    if node_id is not None:
        file_data = create_file.request_newfile(email, node_id)
        if file_data.status_code > 300:
            print("create file error", author_main, file_data.status_code, file_data.text)
            continue
        else:
            file_id = create_preprints.get_files_id(file_data.text)
    ...
    if file_id is not None:
        ...
        if create_result.status_code > 300:
            ...
            preprints_id = create_preprints.get_json_id(create_result.text)
    ...
    #add author_sub as contributors
        if author_sub is not None:
            ...
            for name in result:
                ...
                author_sub_id = create_preprints.get_json_id(author_sub_data.text)
